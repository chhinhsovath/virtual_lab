-- TaRL System - Users and Roles Refactor
-- This migration creates a proper user management system with flexible roles
-- Replaces hardcoded authentication with database-driven user management

BEGIN;

-- Drop existing user_sessions and user_permissions tables (will be recreated)
...
CREATE TABLE IF NOT EXISTS users (
  ...
  -- Optional link to teacher record (for teacher users)
...
CREATE TABLE IF NOT EXISTS user_school_access (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  school_id INTEGER NOT NULL REFERENCES tbl_school_list(sclAutoID),
...
-- Create user_sessions table (updated)
