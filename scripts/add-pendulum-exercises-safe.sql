-- Add sample exercises for Pendulum Lab simulation (safe version that preserves existing data)
DO $$
DECLARE
  sim_id UUID;
  teacher_id UUID;
BEGIN
  ...
  IF sim_id IS NOT NULL THEN
    ...
    IF EXISTS (SELECT 1 FROM simulation_exercises WHERE simulation_id = sim_id) THEN
      RAISE NOTICE 'Exercises already exist for Pendulum Lab. Skipping to preserve existing data.';
    ELSE
