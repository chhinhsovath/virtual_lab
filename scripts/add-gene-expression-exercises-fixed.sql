...
BEGIN
  ...
  -- Get a teacher ID from the teacher information table
  SELECT teiautoid INTO teacher_id FROM tbl_teacher_information LIMIT 1;
  
  -- If no teacher found, create a default one
  IF teacher_id IS NULL THEN
    INSERT INTO tbl_teacher_information (teiname, teinamekh)
    VALUES ('System Teacher', 'គ្រូប្រព័ន្ធ')
    RETURNING teiautoid INTO teacher_id;
  END IF;
